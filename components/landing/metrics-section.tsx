...
const metrics = [
  { 
    ...
    suffix: "+", 
    prefix: "",
    label: "Crores Annual Revenue",
    sublabel: "Empowering Growth with",
  },
  { 
    value: 3, 
    suffix: "+", 
    prefix: "",
    label: "Proprietary Softwares",
    sublabel: "Built on",
  },
  { 
    value: 290, 
  ...
  { 
    ...
    prefix: "",
    label: "Two Decades",
    sublabel: "Delivering Excellence for Over",
  }
];
      ...
      <div className="relative z-10 max-w-[1400px] mx-auto px-6 lg:px-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {metrics.map((metric, index) => (
            <div
              key={index}
              className={`bg-foreground/[0.02] border border-foreground/10 p-8 lg:p-10 flex flex-col justify-between transition-all duration-700 hover:bg-foreground/[0.04] hover:border-foreground/30 ${
                isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-12"
              }`}
              style={{ transitionDelay: `${index * 100}ms` }}
            >
              <div className="text-5xl md:text-6xl lg:text-7xl font-display tracking-tight text-white mb-6 whitespace-nowrap overflow-hidden">
              ...
              <div className="mb-8">
                <DotGraph 
                  ...
                  height={24} 
                  freq1={0.3 + index * 0.05} 
                  freq2={0.1 + index * 0.02} 
                  freqT={0.5 + index * 0.1} 
                  speed={0.018} 
                  baseline={0.35} 
                  amplitude={0.55} 
              ...
              <div>
                ...
                <div className="text-lg text-foreground font-semibold">{metric.label}</div>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className={`mt-16 pt-8 border-t border-foreground/10 flex flex-wrap items-center gap-x-12 gap-y-4 text-sm font-mono text-muted-foreground transition-all duration-1000 delay-500 ${
          ...
          <span>CCTV & Video Analytics</span>
          <span>Biometric Access Control</span>
          <span>ANPR Systems</span>
          <span>Turnstile Barriers</span>
          <span className="text-foreground">+8 more solutions</span>
        </div>
      </div>
    </section>
  );
